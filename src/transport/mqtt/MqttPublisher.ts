 ...
 */
export class MqttPublisher extends StreamWriter {
  ...
  constructor(
    ...
    Logger.debug(`MqttPublisher: ready (broker=${connection.uri})`)
  }
  ...
  async write(data: unknown, topic: string): Promise<void> {
    ...
    try {
      ...
      Logger.warning(`MqttPublisher: write failed on topic '${topic}': ${e}`)
    }
  }
