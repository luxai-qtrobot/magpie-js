 ...
 * MQTT Reader example.
 *
 * Reads from a topic on the free HiveMQ public test broker.
...
async function main() {
  const conn = new MqttConnection(BROKER_URI, { clientId: 'magpie-js-reader-example' })
  ...
  const reader = new MqttStreamReader(conn, { topic: TOPIC })
